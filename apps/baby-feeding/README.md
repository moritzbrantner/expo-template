...
- On first launch, ask which feeding workflows are relevant: direct breastfeeding, bottle feeding, pumping, or any combination.
- Persist that setup locally and keep a `Change setup` action available so mixed-feeding needs can change over time.
...
- Hide bottle-care and record-type controls that are irrelevant to the selected feeding setup.
...
- Keep the most recent feeding event visible without turning the screen into a dashboard of decorative counters.
